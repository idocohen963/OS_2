 ...
 * תיאור כללי של השרת:
 * --------------------
 * atom_warehouse הוא שרת TCP לניהול מלאי של אטומים.
 * השרת מקבל פקודות להוספת אטומים מסוגים שונים:
 * 
 * חיבור TCP:
...
int process_tcp_command(const char *cmd, AtomStock *stock, int client_fd) {
    ...
    if (sscanf(cmd, "%15s %15s %u", op, atom, &amount) == 3 && strcmp(op, "ADD") == 0) {
        ...
        if (atom_adder(stock, atom, amount)) {
            ...
            const char *err_msg = "ERROR: Exceeds MAX_ATOMS or unknown atom type\n";
        ...
        const char *err_msg = "ERROR: Invalid command format\n";
        if (send(client_fd, err_msg, strlen(err_msg), 0) == -1) {
            perror("send to client failed");
        }
    }
...
/**
 * Main function for the atom warehouse server
 * 
